    ...
    Option<DateTime<Utc>>: sqlx::Decode<'r, R::Database> + sqlx::Type<R::Database>,
{
    let enabled: bool = if let Ok(b) = row.try_get::<bool, _>("enabled") {
        ...
        let v: i32 = row.try_get("enabled").map_err(map_err)?;
        v != 0
    };
    let schedule_kind: String = row.try_get("schedule_kind").map_err(map_err)?;
    JobRow {
        job_id: row.try_get("job_id").map_err(map_err)?,
        job_name: row.try_get("job_name").map_err(map_err)?,
        script_name: row.try_get("script_name").map_err(map_err)?,
        script_sig_hash: row.try_get("script_sig_hash").map_err(map_err)?,
        enabled,
        schedule_kind,
        cron_expr: row.try_get("cron_expr").map_err(map_err)?,
        timezone: row.try_get("timezone").map_err(map_err)?,
        run_once_at: row.try_get("run_once_at").map_err(map_err)?,
        run_once_claimed_at: row.try_get("run_once_claimed_at").map_err(map_err)?,
        run_once_claimed_by: row.try_get("run_once_claimed_by").map_err(map_err)?,
        run_once_completed_at: row.try_get("run_once_completed_at").map_err(map_err)?,
        run_once_claim_expires_at: row.try_get("run_once_claim_expires_at").map_err(map_err)?,
        partition_hash: row.try_get("partition_hash").map_err(map_err)?,
        claim_lease_id: row.try_get("claim_lease_id").map_err(map_err)?,
        claim_lease_until: row.try_get("claim_lease_until").map_err(map_err)?,
        pool: row.try_get("pool").map_err(map_err)?,
        ...
        concurrency: row.try_get("concurrency").map_err(map_err)?,
        timeout_ms: row.try_get("timeout_ms").map_err(map_err)?,
        retry_policy_json: row.try_get("retry_policy_json").map_err(map_err)?,
        misfire_policy_json: row.try_get("misfire_policy_json").map_err(map_err)?,
        parent_limits_json: row.try_get("parent_limits_json").map_err(map_err)?,
        next_run_at: row.try_get("next_run_at").map_err(map_err)?,
        current_revision: row.try_get("current_revision").map_err(map_err)?,
