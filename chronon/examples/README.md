...
| [`custom_store_stub.rs`](custom_store_stub.rs) | Decorator `SchedulerStore` wrapping `mem` to validate/audit a couple of calls | `cargo run -p uf-chronon --example custom_store_stub --features mem` | `blank job_name rejected; tick enqueued … run(s)` |
...
| [`authenticated_remote_postgres_redis.rs`](authenticated_remote_postgres_redis.rs) | Postgres+Redis store + Axum admin auth + `RemoteCoordinatorClient`, combined | `cargo run -p uf-chronon --example authenticated_remote_postgres_redis --features postgres,redis,axum` | `missing token denied, x-chronon-admin-token allowed` |
